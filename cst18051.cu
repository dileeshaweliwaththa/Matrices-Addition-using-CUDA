...
int main()
{
    ...
    printf("\n Enter the number of elements of first matrix of size 2 * 3\n");
    for (i = 0; i < 2; i++)
    {
        for (j = 0; j < 3; j++)
        {
    ...
    printf("\n Enter the number of elements of second matrix of size 2 * 3\n");
    for (i = 0; i < 2; i++)
    {
        for (j = 0; j < 3; j++)
        {
    ...
    dim3 grid(3, 2);
    /*We're going to define a two-dimensional Grid(block collection) structure here. 
    The syntax is based on a dim3 grid (no. of columns,no. of rows) */
